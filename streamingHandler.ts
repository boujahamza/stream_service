...
module.exports.createLiveEvent = async (user_id: string) => {
    let uniqueness = user_id;
    let liveEventName = `${uniqueness}`  // WARNING: Be careful not to leak live events using this sample!
    let assetName = `${uniqueness}-`+Date.now();
...
module.exports.startLiveEvent = async (liveEventName: string) => {
    let ingestUrl;
...
module.exports.createStreamingEndpoint = async (assetName: string, manifestName: string) => {
    ...
    let hlsManifest = await buildManifestPaths(scheme, hostname, locator.streamingLocatorId, manifestName);

    return hlsManifest;
}

module.exports.cleanUpResources = async (liveEventName: string, liveOutputName: string) => {
...
module.exports.stopStream = async (liveEventName: string/*, liveOutputName: string*/) => {
    console.log("Cleaning up resources, stopping Live Event billing...")
    console.log("CRITICAL WARNING ($$$$) DON'T WASTE MONEY!: - Wait here for the All Clear - this takes a few minutes sometimes to clean up. DO NOT STOP DEBUGGER yet or you will leak billable resources!")
    await stopStream(liveEventName/*, liveOutputName*/);
    console.log("All Clear, and all cleaned up. Please double check in the portal to make sure you have not leaked any Live Events, or left any Running still which would result in unwanted billing.")
}
...
async function stopStream(liveEventName: string/*, liveOutputName: string*/) {
